...
@section('header', 'Tambah Jadwal Pemeliharaan')

@section('content')
    <div class="mb-4">
        <a href="{{ route('jadwalPemeliharaan.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded inline-flex items-center">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 bg-yellow-50">
            <h2 class="text-lg font-semibold text-yellow-800">Tambah Jadwal Pemeliharaan</h2>
        </div>
        
        <div class="p-6">
            <form action="{{ route('jadwalPemeliharaan.store') }}" method="POST">
                @csrf
                    ...
                    <div>
                        ...
                        <select id="jam" name="jam" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm rounded-md @error('jam') border-red-500 @enderror">
                            ...
                            @foreach ($times as $time)
                                <option value="{{ $time }}" {{ old('jam') == $time ? 'selected' : '' }}>{{ $time }}</option>
                            @endforeach
                        </select>
                        @error('jam')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div class="mb-4">
                    ...
                    <textarea id="keterangan" name="keterangan" rows="4" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm @error('keterangan') border-red-500 @enderror">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
