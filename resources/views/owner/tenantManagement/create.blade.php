...
@section('header', 'Add New Occupant')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-lg font-bold text-orange-500 mb-4">Add New Occupant</h2>

        <form action="{{ route('manajemenPenyewa.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="date" name="ends_on" id="ends_on" required value="{{ old('ends_on') }}"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-yellow-500">
                @error('ends_on')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <select name="status" id="status" required
                    ...
                    <option value="in_arrangement" {{ old('status') == 'in_arrangement' ? 'selected' : '' }}>In Arrangement</option>
            ...
            <div class="mb-6">
                ...
                <select name="payment_status" id="payment_status" required
                    ...
                    <option value="in_arrangement" {{ old('payment_status') == 'in_arrangement' ? 'selected' : '' }}>In Arrangement</option>
                </select>
